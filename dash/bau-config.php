...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_bau_config'])) {
    ...
    try {
        // Atualizar configurações de baú
        foreach ($_POST['nivel'] as $index => $nivel) {
            $indicacoes = $_POST['indicacoes'][$index];
            $premio = str_replace([','], ['.'], $_POST['premio'][$index]);
            $deposito_minimo = str_replace([','], ['.'], $_POST['deposito_minimo'][$index]);
            
            $stmt = $mysqli->prepare("UPDATE bau_config SET indicacoes_necessarias = ?, premio = ?, deposito_minimo = ? WHERE nivel = ?");
            $stmt->bind_param("iddi", $indicacoes, $premio, $deposito_minimo, $nivel);
            $stmt->execute();
            $stmt->close();
        }
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Configurações de baú salvas com sucesso!'
        ]);
        exit;
    } catch (Exception $e) {
        $mysqli->rollback();
...
<body x-data="main"
    ...
    <div class="main-container navbar-sticky flex" :class="[$store.app.navbar]">
        ...
        <div class="main-content flex-1">
            ...
            <div class="h-[calc(100vh-73px)] overflow-y-auto overflow-x-hidden">
                ...
                $bau_config = [];
                $res = $mysqli->query("SELECT * FROM bau_config ORDER BY nivel ASC");
                ...
                <div class="p-4 sm:p-7 min-h-[calc(100vh-145px)]">
                    <form id="form-bau-config" class="space-y-7">
                        <div class="px-2 py-1 mb-4">
                            <h2 class="text-lg font-semibold">Configurações de Baú</h2>
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                Configure os requisitos e prêmios para cada nível de baú do sistema de afiliados.
                            </p>
                        </div>
                            ...
                            <table class="w-full border-collapse">
                                <thead>
                                    <tr class="bg-gray-100 dark:bg-gray-800">
                                        ...
                                        <th class="p-3 text-left">Indicações Necessárias</th>
                                        <th class="p-3 text-left">Depósito Mínimo (R$)</th>
                                        <th class="p-3 text-left">Prêmio (R$)</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php foreach ($bau_config as $config): ?>
                                    <tr class="border-b border-gray-200 dark:border-gray-700">
                                        <td class="p-3">
                                            <input type="hidden" name="nivel[]" value="<?php echo $config['nivel']; ?>">
                                            Nível <?php echo $config['nivel']; ?>
                                        </td>
                                        <td class="p-3">
                                            <input type="number" name="indicacoes[]" class="form-input w-full border border-black/10 dark:border-white/10 bg-transparent px-3 py-2 rounded-lg" value="<?php echo $config['indicacoes_necessarias']; ?>">
                                        </td>
                                        <td class="p-3">
                                            <div class="flex items-center">
                                                ...
                                                <input type="text" name="deposito_minimo[]" class="form-input w-full border border-black/10 dark:border-white/10 bg-transparent px-3 py-2 rounded-r-lg" value="<?php echo formatarValor($config['deposito_minimo']); ?>">
                                            </div>
                                        </td>
                                        <td class="p-3">
                                            <div class="flex items-center">
                                                ...
                                                <input type="text" name="premio[]" class="form-input w-full border border-black/10 dark:border-white/10 bg-transparent px-3 py-2 rounded-r-lg" value="<?php echo formatarValor($config['premio']); ?>">
                                            </div>
                                        </td>
                                    </tr>
                                    <?php endforeach; ?>
                        ...
                        <div class="bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-700 p-4 rounded-lg">
                            <h3 class="text-sm font-medium text-yellow-800 dark:text-yellow-300">Informações importantes</h3>
                            <ul class="mt-2 text-sm text-yellow-700 dark:text-yellow-400 list-disc list-inside">
                                <li>O "Depósito Mínimo" define o valor mínimo que cada indicado deve depositar para contar como uma indicação válida.</li>
                                <li>As "Indicações Necessárias" definem quantos usuários indicados (que depositaram o valor mínimo) são necessários para liberar cada nível de baú.</li>
                                <li>O "Prêmio" é o valor que o afiliado receberá ao abrir o baú do respectivo nível.</li>
                            </ul>
                        </div>
    ...
    <script>
        document.getElementById('form-bau-config')?.addEventListener('submit', function (e) {
            e.preventDefault();
            const form = this;
            const fd = new FormData(form);
            fd.append('ajax_bau_config', 1);
            fetch('bau-config.php', { method: 'POST', body: fd })
                .then(r => r.json())
                .then(data => {
