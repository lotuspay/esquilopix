                ...
                <div class="p-4 sm:p-7 min-h-[calc(100vh-145px)]">
                    <form id="form-valores-config" class="space-y-7">
                        ...
                        <div class="border border-black/10 dark:border-white/10 p-5 rounded-md">
                            ...
                            <div class="border border-black/10 dark:border-white/10 p-5 rounded-md">
                                <div class="mb-5">
                                    <p class="text-sm font-semibold">Multiplicador de Lucro Máximo</p>
                                    <h2 class="text-xs font-italic">Quando o lucro do jogador atinge este multiplicador do valor depositado, o sistema ativa a alta probabilidade de derrota.</h2>
                                </div>
                                <label class="mt-1 flex -space-x-px">
                                    <div class="flex items-center justify-center rounded-l-lg border border-black/10 dark:border-white/10 px-3.5">
                                        <span>X</span>
                                    </div>
                                    <input id="lucro_maximo_multiplicador" name="lucro_maximo_multiplicador" 
                                        class="form-input w-full border border-black/10 dark:border-white/10 bg-transparent px-3 py-2.5" 
                                        placeholder="2.0" type="text" 
                                        value="<?php echo $config['lucro_maximo_multiplicador'] ?? '2.0'; ?>">
                                </label>
                            </div>

                            <div class="border border-black/10 dark:border-white/10 p-5 rounded-md">
                                <div class="mb-5">
                                    <p class="text-sm font-semibold">Probabilidade de Derrota Após Lucro Limite</p>
                                    <h2 class="text-xs font-italic">Probabilidade (%) de gerar posições de raposas para garantir derrota após o jogador atingir o limite de lucro.</h2>
                                </div>
                                <label class="mt-1 flex -space-x-px">
                                    <input id="probabilidade_derrota_apos_lucro" name="probabilidade_derrota_apos_lucro" 
                                        class="form-input w-full border border-black/10 dark:border-white/10 bg-transparent px-3 py-2.5" 
                                        placeholder="95" type="number" min="0" max="100" 
                                        value="<?php echo $config['probabilidade_derrota_apos_lucro'] ?? '95'; ?>">
                                    <div class="flex items-center justify-center rounded-r-lg border border-black/10 dark:border-white/10 px-3.5">
                                        <span>%</span>
                                    </div>
                                </label>
                            ...
                            <div class="border border-black/10 dark:border-white/10 p-5 rounded-md">
                                <div class="mb-5">
                                    <p class="text-sm font-semibold">Depósito Mínimo para Reset</p>
                                    <h2 class="text-xs font-italic">Valor mínimo de depósito necessário para resetar a marcação de limite de lucro.</h2>
                                </div>
                                <label class="mt-1 flex -space-x-px">
                                    <div class="flex items-center justify-center rounded-l-lg border border-black/10 dark:border-white/10 px-3.5">
                                        <span>R$</span>
                                    </div>
                                    <input id="deposito_minimo_reset_lucro" name="deposito_minimo_reset_lucro" 
                                        class="form-input w-full border border-black/10 dark:border-white/10 bg-transparent px-3 py-2.5" 
                                        placeholder="10.00" type="text" 
                                        value="<?php echo $config['deposito_minimo_reset_lucro'] ?? '10.00'; ?>">
                                </label>
                            </div>

                            <div class="border border-black/10 dark:border-white/10 p-5 rounded-md">
                                <div class="mb-5">
                                    <p class="text-sm font-semibold">Multiplicador Máximo Global</p>
                                    <h2 class="text-xs font-italic">Multiplicador máximo que qualquer usuário pode atingir em um jogo. Exemplo: 3.0 = máximo 3x a aposta.</h2>
                                </div>
                                <label class="mt-1 flex -space-x-px">
                                    <div class="flex items-center justify-center rounded-l-lg border border-black/10 dark:border-white/10 px-3.5">
                                        <span>X</span>
                                    </div>
                                    <input id="multiplicador_maximo_global" name="multiplicador_maximo_global" 
                                        class="form-input w-full border border-black/10 dark:border-white/10 bg-transparent px-3 py-2.5" 
                                        placeholder="5.0" type="text" 
                                        value="<?php echo $config['multiplicador_maximo_global'] ?? '5.0'; ?>">
                                </label>
                            </div>
                        </div>
            ...
            fetch('valores.php', { method: 'POST', body: fd })
                ...
                .then(data => {
                    ...
                    if (data.success && data.config) {
                        ...
                        document.getElementById('multiplicador_maximo_global').value = data.config.multiplicador_maximo_global;
